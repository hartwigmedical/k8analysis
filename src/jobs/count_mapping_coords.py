...
class CountMappingCoordsJob(JobABC):
    input_path: GCPPath
    output_path: GCPPath

    @classmethod
    def get_job_type(cls) -> JobType:
        return JobType.COUNT_MAPPING_COORDS
    ...
    def execute(self, service_provider: ServiceProviderABC) -> None:
        ...
        logging.info("Starting count of mapping coords")
        bash_tool_box.count_mapping_coords(local_input_path, local_output_path)
        logging.info("Finished count of mapping coords")

        logging.info("Starting upload of output files")
        gcp_file_cache.upload_from_local(self.output_path)
